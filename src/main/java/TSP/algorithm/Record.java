...
public class Record {
    ...
    public void setOptimaLength(int optimaLength) {
        this.optimaLength = optimaLength;
    }
    ...
    public void add(Route route) {
        synchronized (this) {
            ...
            if (!containsLength(length)) {
                ...
                OutputUtil.println("cost: " + length + "   time:" + (System.currentTimeMillis() - beginTime));
                OutputUtil.println(route.vertexSequence);
            }
    ...
    public boolean isSuccess(Route route) {
        ...
        if(route!=null&&route.getLength()<=optimaLength){
            success=true;
            return true;
        }
        return false;
